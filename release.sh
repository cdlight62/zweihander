...
cat system.template.json | sed -e 's/{{version}}/'"${version}"'/g' -e 's/{{manifest}}/https:\/\/raw.githubusercontent.com\/cdlight62\/blackbirds\/'"${manifest_branch}"'\/zweihander-main\/system.json/g' > system.json
