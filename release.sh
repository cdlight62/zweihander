...
cat system.template.json | sed -e 's/{{version}}/'"${version}"'/g' -e 's/{{manifest}}/https:\/\/raw.githubusercontent.com\/cdlight62\/zweihander\/'"${manifest_branch}"'\/system.json/g' > system.json
